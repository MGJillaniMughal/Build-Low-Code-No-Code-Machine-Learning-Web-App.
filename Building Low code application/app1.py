...
def handle_regression(data, target_variable, test_data_size):
    ...
    if st.button("Run AutoML"):
        ...
        st.write('Accuracy on hold-out: ', best_model_holdout_pred['Label'].astype(float).corr(best_model_holdout_pred['Score'].astype(float), method='spearman'))
    ...
    if st.button("Tune Model"):
        if best_model is not None:
            ...
            st.write('Accuracy on hold-out after tuning: ', tuned_model_holdout_pred['Label'].astype(float).corr(tuned_model_holdout_pred['Score'].astype(float), method='spearman'))
...
def handle_classification(data, target_variable, test_data_size):
    ...
    if st.button("Run AutoML"):
        ...
        st.write('Accuracy on hold-out: ', (best_model_holdout_pred['Label'] == best_model_holdout_pred['Score']).mean())
        st.write('F1 Score on hold-out: ', best_model_holdout_pred['Label'].astype(float).corr(best_model_holdout_pred['Score'].astype(float), method='spearman'))
    ...
    if st.button("Tune Model"):
        if best_model is not None:
            ...
            st.write('Accuracy on hold-out after tuning: ', (tuned_model_holdout_pred['Label'] == tuned_model_holdout_pred['Score']).mean())
            st.write('F1 Score on hold-out after tuning: ', tuned_model_holdout_pred['Label'].astype(float).corr(tuned_model_holdout_pred['Score'].astype(float), method='spearman'))
